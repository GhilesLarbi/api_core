# SaaS Template Public Site — Claude Code Instructions

This is the **SaaS Template public site** — the site every visitor lands on, holding the home page, the auth screens (login, sign up, forgot/reset password) and the signed-in account settings area. It is a **Vite SPA** with TanStack Router, NOT Next.js.

It is **public first**: a page renders for a signed out visitor by default, and only the screens that
need an account (the `/settings` area) are gated. There is no sidebar and no dashboard shell; every
page composes `PublicLayout` with the `Navbar` on top.
...
- **Page width:** every page level block sits in `Container` (`src/components/layout/container.tsx`). Never hand-roll `mx-auto max-w-* px-*`: the navbar and the footer are already measured by it, and a second width means the page no longer lines up with the chrome around it. One width, no variants; if a section genuinely needs another, add it inside `Container`
...
- **Locale-sensitive queries must be tagged `meta: { localized: true }`:** the backend localizes responses via the `Accept-Language` header (sent automatically by `apiClient`), but TanStack Query keys aren't keyed by locale, so cached data doesn't refresh on a language switch by itself. `LanguageProvider` (`packages/ui/src/context/language-provider.tsx`) listens for `languageChanged` and calls `queryClient.invalidateQueries({ predicate: q => q.meta?.localized === true })`. Any new `queryOptions()` whose response contains backend-translated content MUST add `meta: { localized: true }` or it will keep showing stale-language data after a switch. Data that is not translated by the backend (the profile, the session list) should NOT be tagged, to avoid needless refetches and UI flicker.

## Things to Avoid

- Do not use Next.js patterns (server components, App Router, `use server`, `next/` imports)
- Do not call `apiClient` directly in components — use hooks from `src/services/`
